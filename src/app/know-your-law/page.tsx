...
export default function KnowYourLawPage() {
  ...
  const fetchArticles = async (reset = true) => {
    try {
      ...
      const data = await fetchWithLoading<{articles: Article[]}>(
        ...
        {},
        {
          message: reset ? 'Loading legal articles...' : 'Loading more articles...',
          showProgress: false
        }
      );
      ...
      <section className="relative py-16 px-4 sm:px-6 lg:px-8 bg-white/95 backdrop-blur-sm z-20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">Know Your Law</h2>
            <p className="text-lg text-gray-600">Educational content and legal guides to help you understand your rights and the law</p>
          </div>
          ...
          {loading ? (
            ...
            <Card className="text-center py-16">
              <CardContent>
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">No educational content found</h3>
                <p className="text-gray-600 mb-6">
                  {searchTerm ? 'Try adjusting your search terms' : 'No legal education content available yet'}
                </p>
                {searchTerm && (
                  <Button onClick={() => {
                    setSearchTerm('');
            ...
            <div className="text-center mt-12">
              {hasMore ? (
                <Button
                  ...
                  className="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                >
                  {loadingMore ? (
                    <>
          ...
          {!loading && articles.length > 0 && (
            <div className="text-center mt-6">
              <p className="text-sm text-gray-500">
                Showing {articles.length} educational article{articles.length !== 1 ? 's' : ''}
                {!hasMore && ` (all content loaded)`}
              </p>
            </div>
          )}
        </div>
