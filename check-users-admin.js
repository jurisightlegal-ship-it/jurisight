// Check Users in Supabase Database (Admin Access)
// This script uses the service role key to bypass RLS and query all users
...
async function checkUsers() {
  console.log('🔍 Checking users in your Supabase database (Admin Access)...\n');

  try {
    // Query all users with service role (bypasses RLS)
    ...
    const usersWithIssues = users.filter(user => 
      !user.name || !user.email || user.role === null || user.is_active === null
    );

    if (usersWithIssues.length > 0) {
      console.log('\n⚠️  Users with missing required fields:');
      usersWithIssues.forEach(user => {
        console.log(`   - ${user.email || 'No email'}: Missing ${[
          !user.name ? 'name' : null,
          !user.email ? 'email' : null,
          user.role === null ? 'role' : null,
          user.is_active === null ? 'is_active' : null
        ].filter(Boolean).join(', ')}`);
      });
    }
